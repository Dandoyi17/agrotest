...
// Fetch passenger profile info and KYC status
$stmt = $pdo->prepare("SELECT name, profile_photo, kyc_status FROM passengers WHERE id=?");
$stmt->execute([$_SESSION['user_id']]);
$profile = $stmt->fetch();
$profile_img = (!empty($profile['profile_photo']) && file_exists($profile['profile_photo'])) ? $profile['profile_photo'] : 'default_avatar.png';
$profile_name = $profile ? $profile['name'] : 'Passenger';
...
<body>
    <div class="header">
        <div class="profile-header-box">
            ...
            <div class="profile-header-info">
                ...
                <span class="profile-role">Passenger</span>
            </div>
        </div>
        <h2>Welcome to the Passenger's Dashboard</h2>
    </div>
    ...
    <nav class="row">
        <ul class="column side">
            <li>
                <?php if ($kyc_status === 'approved'): ?>
                    <a href="search_trips.php">Find Trip</a>
                <?php else: ?>
                    <span style="color:gray;cursor:not-allowed;">Find Trip (KYC not approved)</span>
                <?php endif; ?>
            </li>
    ...
    <div class="footer">
        <p>Passengers can only book rides after KYC approval</p>
    </div>
</body>
</html>